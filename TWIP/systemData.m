...
function [z_lmi, Acl_pointwise_lmi, Fcl_pointwise_lmi, P_pointwise_lmi] =...
    ...
    xdot_steps_lmi = nPoints(1);
    theta_steps_lmi = nPoints(2);
    thetad_steps_lmi = nPoints(3);
    psid_steps_lmi = nPoints(4);
    
    xdot_lmi = linspace(-stateRange(1), stateRange(1), nPoints(1));
    theta_lmi = linspace(-stateRange(2), stateRange(2), nPoints(2));
    thetadot_lmi = linspace(-stateRange(3), stateRange(3), nPoints(3));
    psidot_lmi = linspace(-stateRange(4), stateRange(4), nPoints(4));   
    ...
    for i = 1:xdot_steps_lmi
        for j = 1:theta_steps_lmi
            ...
            for k = 1:thetad_steps_lmi
               for l = 1:psid_steps_lmi

                %% SDRE - pointwise evaluation and computation of generalised
                %% gradient F
                z_curr = [0.0, xdot_lmi(i), theta_lmi(j), ...
                          thetadot_lmi(k), 0.0, psidot_lmi(l)]';
                [A, B] = systemSDC(z_curr);
                [Q, R] = systemWeightingMatrices(z_curr);   
                dA_dx_x = systemJacobianOpenLoop(z_curr);
                [P, K, ~, ~] = icare(A,B,Q,R,[],[],[]);
                dBKx_dx = systemJacobianStateFeedback(z_curr, ...
                                                 K);                            
                Acl_pointwise_lmi{i,j,k,l} = A - B * K;
                Fcl_pointwise_lmi{i,j,k,l} = A + dA_dx_x - B*K - dBKx_dx;
                P_pointwise_lmi{i,j,k,l} = P;
                z_lmi{i,j,k,l} = z_curr;
                curr_point = l+(k-1)*psid_steps_lmi+...
                    (j-1)*psid_steps_lmi*thetad_steps_lmi+...
                    (i-1)*psid_steps_lmi*thetad_steps_lmi*theta_steps_lmi;         
...
p_lmi = v_steps*theta_steps*thetad_steps*psid_steps; %total number of points for LMIs
...
for i = 1:v_steps    
    for j = 1:theta_steps
        for k = 1:thetad_steps
           ...
           F_curr = Fcl_Array{i,j,k,l};
           Acl_curr = Acl_Array{i,j,k,l};
            %% add postive semi-definite P as pointwise LMI for M(x) to Model
           P_curr = P{i,j,k,l};
           Model = [Model,1e4*eye(n,n)>=replace(M,z,z_curr)>= 1e-3*eye(n,n) ]; %e4,e-5 
